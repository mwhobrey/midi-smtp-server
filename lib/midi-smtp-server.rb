...
module MidiSmtpServer
  ...
  AUTH_MODES = %i[AUTH_FORBIDDEN AUTH_OPTIONAL AUTH_REQUIRED].freeze
  ...
  class Smtpd
    ...
    def stop
      @connectionsMutex.synchronize do
        @tcpServerThread.raise 'stop' if @tcpServerThread
      end
    end
    ...
    def on_auth_event(_ctx, _authorization_id, _authentication_id, _authentication)
          ...
          loop do
            if IO.select([io], nil, nil, 0.1)
              ...
              # Unknown general Exception during processing
                ...
                output = Smtpd500Exception.new.smtpd_result.to_s
              end
    ...
    def process_line(line)
      ...
      # check whether in data or command mode
        ...
        when /^RCPT TO\:/i
          ...
          # ---------
          # check valid command sequence
          raise Smtpd503Exception unless %i[CMD_MAIL CMD_RCPT].include?(Thread.current[:cmd_sequence])
          # check that authentication is enabled
          raise Smtpd530Exception if @auth_mode == :AUTH_REQUIRED && !authenticated?(Thread.current[:ctx])
          # handle command
        ...
        if line.chomp =~ /^\.$/
          ...
          # test all other Exceptions
            ...
            raise Smtpd451Exception, e.to_s
          ensure
    ...
    def process_auth_login_user(encoded_auth_response)
      ...
      '334 ' + Base64.strict_encode64('Password:')
    end
